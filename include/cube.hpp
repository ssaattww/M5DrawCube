...
inline void plot(int x, int y, double brightness, LGFX& renderer){
    renderer.drawPixel(x,y);
}
inline void plot(int x, int y, double brightness, uint8_t r, uint8_t g, uint8_t b, LGFX& renderer){
    renderer.drawPixel(x,y,RGBColor(r,g,b));
}

inline void wuLine(double x0, double y0, double x1, double y1, uint8_t r, uint8_t g, uint8_t b, LGFX& renderer){
...
void drawCube2D(const Cube2D& cube, LGFX& renderer)
{
 ...
 * LCD instance
 */
inline void cube(float roll, float pitch, float yaw, float x, float y, float z, LGFX lcd){
